...
#### **NMAR Analysis**
There are only three columns in the merged DataFrame with missing values -- those being 'description', 'review', and 'rating'/'avg_rating'. I believe these three columns to not be NMAR (Not Missing at Random). This is because columns like description or review commonly have values that are missing because the author of the review or the rating simply chose not to write a description or review; this makes them MD (Missing by Design). The missing ratings are a different case. Users interacting with a recipe cannot simply choose to not rate the recipe they are interacting with as the interactions DataFrame is built around those ratings. Because of this, I believe the missing values in the 'rating' and 'avg_rating' column could be MAR (Missing at Random).
